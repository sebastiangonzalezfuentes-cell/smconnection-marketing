    ...
    >
      ...
      <div className="relative mx-auto max-w-3xl flex flex-col items-center gap-8 text-center">
        ...
        <p className="text-sm font-semibold uppercase tracking-wider text-blue-400">
          ¿Tienes dudas de por cuál empezar?
        </p>

        {/* Headline */}
        <h2
          className="font-bold tracking-tight text-white text-balance"
          style={{ fontSize: 'clamp(1.875rem, 3vw + 1rem, 3rem)' }}
        >
          Una llamada de 30 minutos{' '}
          <GradientText>resuelve todo.</GradientText>
        </h2>

        {/* Descripción */}
        <p className="max-w-xl text-neutral-400 leading-relaxed">
          Te decimos qué plan se ajusta a tu situación, qué automatizaríamos primero
          y cuánto podrías recuperar. Sin compromiso, sin presión, sin costo.
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
          ...
          <Button variant="whatsapp" size="lg" asChild className="rounded-full">
            <Link
              href={SITE_CONFIG.whatsappUrl(
                'Hola, quiero saber qué plan es el mejor para mi empresa.',
              )}
              target="_blank"
              rel="noopener noreferrer"
            >
        ...
        <div className="flex items-start gap-3 rounded-radius-xl bg-white/5 border border-white/10 px-5 py-4 text-left max-w-xl">
          <ShieldCheck
            className="size-5 shrink-0 text-blue-400 mt-0.5"
            aria-hidden="true"
          />
          <p className="text-sm text-neutral-400 leading-relaxed">
            <span className="font-semibold text-neutral-200">Garantía de valor:</span>{' '}
            Si después del Discovery Sprint no ves valor claro en automatizar,
            te devolvemos lo invertido. Sin letra chica.
          </p>
        </div>
      </div>
    </section>
